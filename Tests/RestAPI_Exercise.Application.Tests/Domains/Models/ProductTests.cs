...
{
    // ヘルパー：有効なカテゴリ
    private ProductCategory CreateCategory(string name = "雑貨") =>
    ...
    public void EmptyProductName_ShouldThrowDomainException()
    {
        ...
        var ex = Assert.ThrowsException<DomainException>(() =>
        {
            _ = new Product(Guid.NewGuid().ToString(), "", 100, category, stock);
        });
        // 例外メッセージを検証する
    ...
    public void CategoryNameLongerThan30Chars_ShouldThrowDomainException()
    {
        ...
        var ex = Assert.ThrowsException<DomainException>(() =>
        {
            _ = new Product(Guid.NewGuid().ToString(), name, 100, category, stock);
        });
        // 例外メッセージを検証する
    ...
    public void UnitPrice_WithNegativeValue_ShouldThrowDomainException()
    {
        ...
        var ex = Assert.ThrowsException<DomainException>(() =>
        {
            _ = new Product(Guid.NewGuid().ToString(), "正しい商品", -1, category, stock);
        });
        // 例外メッセージを検証する
    ...
    public void ProductName_WithValidValue_ShouldSucceed()
    {
        ...
        var stock = CreateStock();
        // インスタンスを生成する
        var product = new Product("旧商品", 500, category, stock);
        // 商品名を変更する
    ...
    public void ProductPrice_WithValidValue_ShouldSucceed()
    {
        ...
        var stock = CreateStock();
        // インスタンスを生成する
        var product = new Product("商品", 500, category, stock);
        // 単価を変更する
    ...
    public void Equals_WithSameUuid_ShouldReturnTrue()
    {
        ...
        // インスタンスを生成する
        var uuid = Guid.NewGuid().ToString();
        var p1 = new Product(uuid, "A", 100, category, stock);
        var p2 = new Product(uuid, "B", 200, category, stock);
        // 等価性を検証する
    ...
    {     
        ...
        var stock = CreateStock();
        // インスタンスを生成する
        var p1 = new Product("A", 100, category, stock);
        var p2 = new Product("B", 200, category, stock);
        // 等価性を検証する
        var result = p1.Equals(p2);
        // 非等価であることを評価する
