...
public class RegisterProductUsecaseTests
{
    ...
    public async Task GetCategoriesAsync_ShouldReturnAllCategories()
    {
        ...
        // 取得内容を検証する
        Assert.AreEqual("a1f70bb5-aac0-4f3e-95a9-712dc100a26d", categories[0].CategoryUuid);
        Assert.AreEqual("文房具", categories[0].Name);
        Assert.AreEqual("45fc0f1f-b062-46c8-89eb-6a78741b390f", categories[1].CategoryUuid);
        Assert.AreEqual("雑貨", categories[1].Name);
        Assert.AreEqual("7651f725-276b-4899-b20b-a6e6baaf63d1", categories[2].CategoryUuid);
    ...
    {
        // 商品カテゴリ雑貨を取得する
        var category = await _uscase!.GetCategoryByIdAsync("45fc0f1f-b062-46c8-89eb-6a78741b390f");
        // nullでないことを検証する
        Assert.IsNotNull(category);
        // 商品カテゴリIdと商品カテゴリ名を検証する
        Assert.AreEqual("45fc0f1f-b062-46c8-89eb-6a78741b390f", category.CategoryUuid);
    ...
    {
        // テストデータを用意する
        var category = new ProductCategory("a1f70bb5-aac0-4f3e-95a9-712dc100a26d", "文房具");
