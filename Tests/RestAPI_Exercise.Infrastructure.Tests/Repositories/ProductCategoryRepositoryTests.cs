...
public class ProductCategoryRepositoryTests
{
    ...
    public async Task SelectAllAsync_ShouldReturnAllCategories()
    {
        ...
        // 取得内容を検証する
        Assert.AreEqual("a1f70bb5-aac0-4f3e-95a9-712dc100a26d", categoryies[0].CategoryUuid);
        Assert.AreEqual("文房具", categoryies[0].Name);
        Assert.AreEqual("45fc0f1f-b062-46c8-89eb-6a78741b390f", categoryies[1].CategoryUuid);
        Assert.AreEqual("雑貨", categoryies[1].Name);
        Assert.AreEqual("7651f725-276b-4899-b20b-a6e6baaf63d1", categoryies[2].CategoryUuid);
    ...
    public async Task SelectByIdAsync_WhenIdExists_ShouldReturnCategory()
    {
        var category = await _productCategoryRepository
            .SelectByIdAsync("a1f70bb5-aac0-4f3e-95a9-712dc100a26d");
        // nullでないことを検証する
        Assert.IsNotNull(category);
        // 取得内容を検証する
        Assert.AreEqual("a1f70bb5-aac0-4f3e-95a9-712dc100a26d", category.CategoryUuid);
